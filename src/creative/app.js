...
function renderPlayer(side){
  ...
  const special=role.id==='lv-bu'?`<div class="role-state fury"><b>霸气</b><span>${[1,2,3].map(n=>`<i class="${n<=state.fury?'filled':''}"></i>`).join('')}</span><small>${state.fury}/3</small></div>`:role.id==='zhao-yun'?`<div class="role-state zhao"><b>愈厉累计</b><span>${CREATIVE_LEVELS.map(level=>`<i>${level}级 ${state.placedByLevel[level]??0}/2</i>`).join('')}</span></div>`:'';
