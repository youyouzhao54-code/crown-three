...
import { CREATIVE_LEVELS, createCreativeGame, legalCreativeMoves, legalCreativePlacements, moveCreativePiece, placeCreativePiece, recycleCreativePiece, resolveLvBuEnd, skipCreativeBonus, skipLvBuNoAction, skipZhangFeiBonus, topCreativePiece } from './engine.js';
...
function renderGame(){
  ...
  $('#creative-status').textContent=game.winner?`${effect}${game.winner==='black'?'黑方':'白方'}${winCount}子连珠，获得胜利`:game.phase==='no-action'?`${effect}霸气不足2点，本回合不能行动`:game.phase==='lvbu-end'?`${effect}【单三】点击己方一枚5级以上棋子发动，或选择不发动`:game.phase==='zhangfei-bonus'?`${effect}【狂啸】可以继续行动，或主动结束追加行动`:game.phase==='bonus'?`${effect}${game.currentPlayer==='black'?'黑方':'白方'}：操作一枚低于 ${game.bonusMaxRank} 级的棋子，或放弃`:`${effect}第 ${game.turn} 回合 · ${game.currentPlayer==='black'?'黑方':'白方'}行动 · ${action}`;
}
