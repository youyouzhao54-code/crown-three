...
import { canCreativeSuppress, resolveCreativeAfterAction, resolveCreativeRecycle, resolveCreativeSuppress, resolveCreativeTurnStart } from './skills.js';
...
export function createCreativeGame(blackRole, whiteRole) {
  ...
  const game={
    ...
    players: {
      black: { roleId:blackRole.id, skillId:blackRole.skill.id, skillState:{placedByLevel:{},fury:blackRole.skill.id==='lv-bu'?1:0,actionPaid:false}, inventory:inventoryOf(blackRole) },
      white: { roleId:whiteRole.id, skillId:whiteRole.skill.id, skillState:{placedByLevel:{},fury:whiteRole.skill.id==='lv-bu'?1:0,actionPaid:false}, inventory:inventoryOf(whiteRole) },
    },
...
export function placeCreativePiece(state,index,level) {
  ...
  if(target)resolveCreativeSuppress(game,{player,index,piece,target});
